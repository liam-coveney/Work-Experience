...
# Our changes from borrowed file
# Fixed bugs with resource counts
# implemented getAllPossibleMoves, which returns a list of move tuples for each action a player might take
# Fixed broken human trade logic
# Implemented AI trading with bank, ports, and other players
# Implemented AI ability to make moves based on a move tuple
# Fixed logic of AI player moves to be more consistent with the game 
#Class definition for a player
class player():
    'Class Definition for Game Player'

    #Initialize a game player, we use A, B and C to identify
    def __init__(self, playerName, playerColor, isAI = False):
        ...
        # self.resources = {'ORE':100, 'BRICK':100, 'WHEAT':100, 'WOOD':100, 'SHEEP':100}
        ...
        #Each of the 3 lists store vertex information - Roads are stored with tuples of vertex pairs
    ...
    def getAllPossibleMoves(self, state):
        # movesDict is {string:list} dictionary of each possible move and a list of tuples, where the tuple includes necessary information for making that move.
        # For example, to build a road, one must eventually call player.build_road(v1, v2, board). Therefore, movesDict["buildRoad"] is a list [(v1_1, v2_1), (v1_2, v2_2), ... etc.]
        # movesDict = {"buildRoad": [], "buildSettlement": [], "buildCity": [], "buyDev": [], "playDev": [], "proposeTrade": []}
        movesDict = {"playDev": [], "proposeTrade": [], "proposeTradeBank": []}

        board = state.static.board

        if self.resources["WOOD"] > 0 and self.resources["BRICK"] > 0 and self.roadsLeft > 0:
            movesDict["buildRoad"] = [vertices for vertices in board.get_potential_roads(self).keys()]

        if self.resources["WOOD"] > 0 and self.resources["BRICK"] > 0 and self.resources["SHEEP"] > 0 and self.resources["WHEAT"] > 0 and self.settlementsLeft > 0:
            movesDict["buildSettlement"] = [vCoord for vCoord in board.get_potential_settlements(self).keys()]

        if self.resources["ORE"] >= 3 and self.resources["WHEAT"] >= 2 and self.citiesLeft > 0:
            movesDict["buildCity"] = [vCoord for vCoord in board.get_potential_cities(self).keys()]

        if self.resources["ORE"] > 0 and self.resources["SHEEP"] > 0 and self.resources["WHEAT"] > 0 and sum(board.devCardStack.values()) > 0:
            movesDict["buyDev"] = True

        maxResource = max(self.resources, key=self.resources.get)
        numMaxResource = self.resources.get(maxResource)

        for resourceToGet in list(self.resources.keys()):
            if self.resources[resourceToGet] == 0:
                movesDict["proposeTrade"].append((resourceToGet, maxResource))

        for resourceToGet in list(self.resources.keys()):
            if self.resources[resourceToGet] == 0 and numMaxResource >= 4:
                movesDict["proposeTradeBank"].append((resourceToGet, maxResource, 4))

        for port in self.portList:
            if port == '3:1 PORT':
                for resourceToGet in list(self.resources.keys()):
                    if self.resources[resourceToGet] == 0 and numMaxResource >= 3:
                        movesDict["proposeTradeBank"].append((resourceToGet, maxResource, 3))
            elif port == 'SHEEP 2:1':
                if self.resources['SHEEP'] >= 2:
                    for resource in list(self.resources.keys()):
                        movesDict["proposeTradeBank"].append((resource, 'SHEEP', 2))
            elif port == 'BRICK 2:1':
                if self.resources['BRICK'] >= 2:
                    for resource in list(self.resources.keys()):
                        movesDict["proposeTradeBank"].append((resource, 'BRICK', 2))
            elif port == 'WOOD 2:1':
                if self.resources['WOOD'] >= 2:
                    for resource in list(self.resources.keys()):
                        movesDict["proposeTradeBank"].append((resource, 'WOOD', 2))
            elif port == 'ORE 2:1':
                if self.resources['ORE'] >= 2:
                    for resource in list(self.resources.keys()):
                        movesDict["proposeTradeBank"].append((resource, 'ORE', 2))
            elif port == 'WHEAT 2:1':
                if self.resources['WHEAT'] >= 2:
                    for resource in list(self.resources.keys()):
                        movesDict["proposeTradeBank"].append((resource, 'WHEAT', 2))
            
        if not state.devCardPlayedThisTurn:
            for devCard in self.devCards:
                if self.devCards[devCard] > 0 and not devCard == "VP":
                    movesDict["playDev"].append(devCard)

        playableMoves = [("endTurn", None)]

        for moveType, info in movesDict.items():
            if not moveType == "buyDev":
                for moveInfo in movesDict[moveType]:
                    playableMoves.append((moveType, moveInfo))
            elif movesDict["buyDev"] == True:
                playableMoves.append(("buyDev", None))
    ...
    def steal_resource(self, player_2):
        ...
        if sum(player_2.resources.values()) == 0:
            return
    ...
    def initiate_trade(self, game, trade_type):
        ...
        if trade_type == 'BANK':
            ...
            while accept not in ['Y', 'N']:
                accept = input(f"Does {playerToTrade_name} accept the trade? (Y/N)")
    ...
    def aiBankTrade(self, moveInfo, state):
        resourceToGet = moveInfo[0]
        resourceToGive = moveInfo[1]
        numberToGive = moveInfo[2]

        self.resources[resourceToGet] += 1
        self.resources[resourceToGive] -= numberToGive
        
        
    def aiProposeTrade(self, trade: tuple, state):
        for player in state.dynamic.players:
            if not self == player and player.resources[trade[0]] > 1:
                if player.acceptAITrade(self, trade, state):
                    self.resources[trade[0]] += 1
                    self.resources[trade[1]] -= 1

                    player.resources[trade[1]] += 1
                    player.resources[trade[0]] += 1

    def acceptAITrade(self, player, trade, state):
        if not self.isAI:
            accept = ""
            while not (accept == "Y" or accept == "N"):
                accept = input(f"Give {player.name} 1 {trade[0]} for 1 {trade[1]} (Y/N)? ")
            return True if accept == "Y" else False
        else:
            if self.resources[trade[0]] < self.resources[trade[1]] - 1:
                return True
            else: return False

    def buildRoad(self, v1, v2, board):
        self.build_road(v1, v2, board)
    
    def buildSettlement(self, vCoord, board):
        self.build_settlement(vCoord, board)
    
    def buildCity(self, vCoord, board):
        self.build_city(vCoord, board)
    
    def buyDev(self, board):
        self.draw_devCard(board)

    def playDev(self, card, gameState):
        board = gameState.static.board
        #Logic for each Dev Card
        if(card == 'KNIGHT'): 
            self.moveRobber(board)
            self.knightsPlayed += 1 

        if(card == 'ROADBUILDER'):
            possibleRoads = [vertices for vertices in board.get_potential_roads(self).keys()]
            roadToBuild = random.choice(possibleRoads)
            self.buildRoad(roadToBuild[0], roadToBuild[1], board)

        if(card == 'YEAROFPLENTY'):
            for i in range(2):
                resource = min(self.resources, key=self.resources.get)
                self.resources[resource] += 1

        if(card == 'MONOPOLY'):
            resourceToMonopolize = min(self.resources, key=self.resources.get)

            for player in gameState.dynamic.players:
                if(player != self):
                    numLost = player.resources[resourceToMonopolize]
                    player.resources[resourceToMonopolize] = 0
                    self.resources[resourceToMonopolize] += numLost
    ...
    def endTurn(self, state):
        index = state.dynamic.players.index(self)
        nextPlayer = (index + 1) % len(state.dynamic.players)
        state.dynamic.currPlayer = state.dynamic.players[nextPlayer]
        state.devCardPlayedThisTurn = False

    def makeMove(self, moveType, moveInfo, state):
        if moveType == "buildRoad":
            self.buildRoad(moveInfo[0], moveInfo[1], state.static.board)
        elif moveType == "buildSettlement":
            self.buildSettlement(moveInfo, state.static.board)
        elif moveType == "buildCity":
            self.buildCity(moveInfo, state.static.board)
        elif moveType == "buyDev":
            self.buyDev(state.static.board)
        elif moveType == "playDev":
            self.playDev(moveInfo, state)
            state.devCardPlayedThisTurn = True
        elif moveType == "endTurn":
            self.endTurn(state)
        elif moveType == "proposeTrade":
            self.aiProposeTrade(moveInfo, state)
        elif moveType == "proposeTradeBank":
            self.aiBankTrade(moveInfo, state)
        state.lastMove = (moveType, moveInfo)
